...
("sk_test_51N1nURSJ0heUjZB8De7lFuFEA303FF1viJuVdRuWas55BBjY3U3evQUtsXTaiGO4TPrnk5Zq6dZXeeoEyEiGP0nQ009F8Nne9L");
...
app.post("/registeremp", async (req, resp) => {
  ...
  const mailOptions = {
    ...
    html:
      "<p>To login into your account</p><p>Your Username is your <b>Email ID</b> & </p> Your password is </p><b>" +
      pass +
      "</b>",
  };
  let result = await user.save();
  ...
  let user = await User.findByIdAndUpdate(req.body.id, {
    ...
    .then((response) => {
      if (!response) {
        ...
        let result = User.findByIdAndUpdate(req.body.id, { otp: "" })
          .then((response) => {
            return resp.send(response);
          })
          .catch((error) => {
...
app.post("/verifyemail", async (req, resp) => {
  ...
  let usersDetails = await User.findOneAndUpdate(
    { email: req.body.email },
    { otp: otp }
  );
  ...
  if (usersDetails) {
    const mailOptions = {
      ...
      html:
        '<a href="http://localhost:3000/otpverification"><p>Please Verify your email address to complete the signup and login into your account.</p><p>This link expires in 6 hours</p><p>Press to Procced.</p></a>' +
        otp,
    };
    transporter.sendMail(mailOptions);
  }
...
app.get("/appointment_status/:bid", async (req, resp) => {
  console.log(req.body);
  let result = await Appointment.find({ status: "0", barber: req.params.bid })
    .populate("customerid")
    .populate("barber");
  console.log(result);
  if (result) {
